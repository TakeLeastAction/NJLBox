    ...
    integer,parameter :: NumPar= 8000, Ntest=200, NCenX=0,NCenY=0,NCenZ=0                         !,NCenZ=3  NumPar: maximum number of particles, Ntest: maximum number of test particle
                                                                                          ! 7.7GeV NumPar=2000,Ntest=1000
    integer,parameter :: Ngx = 21, Ngy = 21, Ngz = 21, NgN = 150                                              ! must be larger than nx,ny,nz
!********************************* particle ******************************
...
!******************************* saving ***************************************  
    open (unit=17, file='zpc_coll.dat', status='unknown')                                         !changed by sunkj  
    open (unit=100, file='zpc_final.dat', status='unknown')    !------------------------------- Final state quark information   
    ...
    read(5,*)Nixj
    close(5)
    open(6,file='NJLNevent.txt')          !number of test particle
    read(6,*)nevent
    close(6)

!***************************** Control Parameter ********************************
	qLen = 10.0  ! box length  [-qLen, qLen]  
	IperiodicBound = 1  ! periodic boundary
    ...
    do npara=1, nevent !<----------------------------------------------------- new event
       ...
       do npartition=1, npart
          ...
          do i=1, ipart(npara)
             ...
             read(17,*) id,xx,yy,zz,qx,qy,qz,xmass,time
...
99  continue
	...
	do npara=1, nevent
       write(100,"(I7)") ipart(npara)
	   do i=1, ipart(npara)

          write(100,2013) int(q(9,i,npara)),q(1,i,npara),q(2,i,npara),q(3,i,npara), &
		  q(5,i,npara),q(6,i,npara),q(7,i,npara),q(8,i,npara),t 
