...
clin-9/29/03 changed name in order to distinguish from /prec2/
     ...
     &       XMAR(MAXSTR)
          COMMON /ARPRCXJ/ ITYPARXJ(MAXSTR,MAXR),
     &       GXARXJ(MAXSTR,MAXR), GYARXJ(MAXSTR,MAXR)
     &, GZARXJ(MAXSTR,MAXR), FTARXJ(MAXSTR,MAXR),
     &       PXARXJ(MAXSTR,MAXR), PYARXJ(MAXSTR,MAXR)
     &, PZARXJ(MAXSTR,MAXR), PEARXJ(MAXSTR,MAXR),
     &       XMARXJ(MAXSTR,MAXR)
ccbz11/11/98
...
cc      SAVE /ilist8/
          COMMON /precnjl/GXnjl(MAXPTN,MAXR),GYnjl(MAXPTN,MAXR) !!kjsun
     &,GZnjl(MAXPTN,MAXR),FTnjl(MAXPTN,MAXR)
     &,PXnjl(MAXPTN,MAXR), PYnjl(MAXPTN,MAXR), PZnjl(MAXPTN,MAXR)
     &,Enjl(MAXPTN,MAXR),XMASSnjl(MAXPTN,MAXR), ITYPnjl(MAXPTN,MAXR) !xj: njl
      DOUBLE PRECISION  GXnjl, GYnjl, GZnjl, FTnjl
     &, PXnjl, PYnjl, PZnjl, Enjl, XMASSnjl
      COMMON /ilistnjl/ LSTRGnjl(MAXPTN,MAXR), LPARTnjl(MAXPTN,MAXR) !xj: njl
	COMMON /NJLMUL/ NJLMUL(MAXR) !xj: njl
      !COMMON /ARERC1/MULTI1(MAXR) !XJ: ART   !!kjsun
...
cc      SAVE /SOFT/
      ...
      COMMON/SOFTXJ/PXSGSXJ(MAXSTR,3,MAXR),PYSGSXJ(MAXSTR,3,MAXR)
     &,PZSGSXJ(MAXSTR,3,MAXR),PESGSXJ(MAXSTR,3,MAXR),
     &PMSGSXJ(MAXSTR,3,MAXR),GXSGSXJ(MAXSTR,3,MAXR),
     &GYSGSXJ(MAXSTR,3,MAXR),GZSGSXJ(MAXSTR,3,MAXR),
     &FTSGSXJ(MAXSTR,3,MAXR),K1SGSXJ(MAXSTR,3,MAXR),
     &K2SGSXJ(MAXSTR,3,MAXR),NJSGSXJ(MAXSTR,MAXR) !mixed event coalescence
cc      SAVE /SOFTXJ/
clin-4/26/01 lepton and photon info:
     ...
     &       XMN(MAXIDL)
cc      SAVE /NOPREC/
        COMMON /NOPRECXJ/ NNOZPCXJ(MAXR), ITYPNXJ(MAXIDL,MAXR), !!kjsun
     &       GXNXJ(MAXIDL,MAXR), GYNXJ(MAXIDL,MAXR), GZNXJ(MAXIDL,MAXR)
     &, FTNXJ(MAXIDL,MAXR),
     &       PXNXJ(MAXIDL,MAXR), PYNXJ(MAXIDL,MAXR), PZNXJ(MAXIDL,MAXR)
     &, EENXJ(MAXIDL,MAXR),
     &       XMNXJ(MAXIDL,MAXR)	!XJ TEST PARTICLE    !!kjsun
clin-6/22/01:
...
clin-7/2011 ioscar value is needed:
        ...
        common /para7xj/njlioscar(MAXR),njlnsmm0(MAXR),njlnsmb0(MAXR)
     & ,njlnsmab0(MAXR),njlnsmm1(MAXR),njlnsmb1(MAXR),njlnsmab1(MAXR)		
...
clin-4/19/01 convert hadrons to partons for ZPC (with GX0 given):
        call htop
		nnozpcxj(ixj) = nnozpc  !kjsun
        !NJLMul(ixj) = MUL !kjsun
...
c.....call ZPC for parton cascade
ckjsun        CALL ZPCMN
           call GETNJLFeng(IXJ) !XJ: TRANSFER PARTON DATA
		   
		   njlioscar(ixj) = isoscar
		   njlnsmm0(ixj) = nsmm0
		   njlnsmb0(ixj) = nsmb0
		   njlnsmab0(ixj) = nsmab0
		   
		   !  particles not in zpc  (gethadron also need to change)
		   DO I = 1, nnozpcxj(ixj) 
			     FTN(I) = 1e-20
		         GXNxj(I,ixj) = GXN(I)
			     Itypnxj(I,ixj) = ITYPN(I)
				 GYNxj(I,ixj) = GYN(I)
				 GZNxj(I,ixj) = GZN(I)
				 PXNxj(I,ixj) = PXN(I)
				 PYNxj(I,ixj) = PYN(I)
...
2019        enddo
           ...
           !call NJLFeng  !kjsun
      open(unit=98, file='NJLEVE.dat', status='unknown')
      write(98,*)NUM
            call system('./NJLballrun.sh')	
		   write(*,*)"dear master, NJL evolution done !"
           call GETNJLFengBack(NUM) !XJ: TRANSFER PARTON DATA	BACK		
		   write(*,*)"dear master, NJL copy data back done !"
c    save partonic information     c
        	DO IXJ = 1,NUM      
clin-6/2009:
c        WRITE (14, 395) ITEST, MUL, bimp, NELP,NINP,NELT,NINTHJ
...
ckjsun           if(dmax1(abs(GX5(I)),abs(GY5(I)),abs(GZ5(I)),abs(FT5(I)))
ckjsun     1          .lt.9999) then
ckjsun              write(14,210) ITYP5(I), PX5(I), PY5(I), PZ5(I), XMASS5(I),
ckjsun     1             GX5(I), GY5(I), GZ5(I), FT5(I)
ckjsun           else
ckjsun              write(14,211) ITYP5(I), PX5(I), PY5(I), PZ5(I), XMASS5(I),
ckjsun     1             GX5(I), GY5(I), GZ5(I), FT5(I)
ckjsun           endif
c


           WRITE (14, 512) ITYPnjl(I,ixj),XMASSnjl(I,ixj)
     &,GXnjl(I,ixj),GYnjl(I,ixj),GZnjl(I,ixj),FTnjl(I,ixj)
     &,PXnjl(I,ixj),PYnjl(I,ixj),PZnjl(I,ixj),Enjl(I,ixj) 
     &,LSTRGnjl(I,ixj), LPARTnjl(I,ixj)
 1016   CONTINUE
c 511    FORMAT(1X, 3F10.4, I6, 2F10.4)
 512    FORMAT(I6,9(1X,f10.3),1X,I6,1X,I3)
...
ckj              FTSGS(I, J) = 0d0
 ...
 1018   CONTINUE
        ...
        DO 1019 I = 1, njlmul(ixj) !MUL
ckjsun           IITYP=ITYP5(I)
ckjsun           NSTRG = LSTRG1(I)
ckjsun           NPART = LPART1(I)
           IITYP=ITYPnjl(I,ixj)


           NSTRG = LSTRGnjl(I,ixj)
	    	 NJSGSXJ(NSTRG, IXJ) = NJSGSXJ(NSTRG, IXJ) + 1 !XJ
           NPART = LPARTnjl(I,ixj)
		   
           K2SGSXJ(NSTRG, NPART, IXJ) = ITYPnjl(I,ixj)
           PXSGSXJ(NSTRG, NPART, IXJ) = PXnjl(I,ixj)
           PYSGSXJ(NSTRG, NPART, IXJ) = PYnjl(I,ixj)
           PZSGSXJ(NSTRG, NPART, IXJ) = PZnjl(I,ixj)
           PMSGSXJ(NSTRG, NPART, IXJ) = XMASSnjl(I,ixj)
ckjsun          K2SGS(NSTRG, NPART) = ITYP5(I)
ckjsun           PXSGS(NSTRG, NPART) = PX5(I)
ckjsun           PYSGS(NSTRG, NPART) = PY5(I)
ckjsun           PZSGS(NSTRG, NPART) = PZ5(I)
ckjsun           PMSGS(NSTRG, NPART) = XMASS5(I)
...
ckjsun           E5(I)=dsqrt(PX5(I)**2+PY5(I)**2+PZ5(I)**2+XMASS5(I)**2)
ckjsun           PESGS(NSTRG, NPART) = E5(I)
		   E5(I) = Enjl(I,ixj)
		   PESGSXJ(NSTRG, NPART, IXJ) = Enjl(I,ixj)
c           if(abs(PZ5(i)/E5(i)).gt.0.9999999d0) 
c     1          write(91,*) 'b: new E5(I)=',E5(i)
clin-7/20/01-end
ckjsun           GXSGS(NSTRG, NPART) = GX5(I)
ckjsun           GYSGS(NSTRG, NPART) = GY5(I)
ckjsun           GZSGS(NSTRG, NPART) = GZ5(I)
ckjsun           FTSGS(NSTRG, NPART) = FT5(I)
		   
           GXSGSXJ(NSTRG, NPART, IXJ) = GXnjl(I,ixj)
           GYSGSXJ(NSTRG, NPART, IXJ) = GYnjl(I,ixj)
           GZSGSXJ(NSTRG, NPART, IXJ) = GZnjl(I,ixj)
           FTSGSXJ(NSTRG, NPART, IXJ) = FTnjl(I,ixj)		   
 1019   CONTINUE
ckjsun        CALL HJANA2
        enddo !kjsun
clin-4/19/01-end
         ...
         print *,"transfer parton information to hadronization:",ixj		
           ...
           EATT=0.
		      DO I = 1, njlmul(ixj) 
				 ...
				 E5(I) = Enjl(I,ixj)
			  enddo
		   isoscar =njlioscar(ixj)
		   nsmm0=njlnsmm0(ixj) 
		   nsmb0=njlnsmb0(ixj) 
		   nsmab0=njlnsmab0(ixj) 
         ...
         print *,"hadron resample begin, events=",ixj		   
          ...
          print *,"neutron resample, events=",ixj,I,ixj1,I1	
            ...
            enddo	
              ...
              pearxj(I,ixj)=sqrt(xmarxj(I,ixj)**2.+pxarxj(I,ixj)**2.
     &        +pyarxj(I,ixj)**2.+pzarxj(I,ixj)**2.)
...
cc
	DO IXJ = 1,NUM     !recording spectators
           do 1006 I=1,nnozpcxj(ixj)
              ...
              PZARXJ(nattxj1,IXJ)=PZNxj(I,ixj)
              PEARXJ(nattxj1,IXJ)=EENxj(I,ixj)
              XMARXJ(nattxj1,IXJ)=XMNxj(I,ixj)
			  
clin-2/2017 this may cause no particle record in ampt.dat for the event:
c      if(FTNxj(I,ixj).le.0) write(*,*) 'spect,ERROR:FT0<=0',I 
c     1 ,FTNxj(I,ixj),ITYPNxj(I,ixj),GXNxj(I,ixj),GYNxj(I,ixj)
c     2 ,GZNxj(I,ixj),PXNxj(I,ixj),PYNxj(I,ixj),PZNxj(I,ixj)
c     3 ,XMNxj(I,ixj)	 
 1006      continue
...
ckj           do 1006 I=1,nnozpc
ckj              NATT=NATT+1
ckj              KATT(NATT,1)=ITYPN(I)
ckj              PATT(NATT,1)=PXN(I)
ckj              PATT(NATT,2)=PYN(I)
ckj              PATT(NATT,3)=PZN(I)
ckj              PATT(NATT,4)=EEN(I)
ckj              EATT=EATT+EEN(I)
ckj              GXAR(NATT)=GXN(I)
ckj              GYAR(NATT)=GYN(I)
ckj              GZAR(NATT)=GZN(I)
ckj              FTAR(NATT)=FTN(I)
ckj              ITYPAR(NATT)=ITYPN(I)
ckj              PXAR(NATT)=PXN(I)
ckj              PYAR(NATT)=PYN(I)
ckj              PZAR(NATT)=PZN(I)
clin-12/2016:
ckj              if(EEN(I).lt.0) then
ckj                write(6,*) 'E<0 in HIJING, recalculated4:',I,
ckj     1                XMN(I),PXN(I),PYN(I),PZN(I),EEN(I)
ckj                 EEN(I)=sqrt(XMN(I)**2+PXN(I)**2+PYN(I)**2+PZN(I)**2)
ckj              endif
c
ckj              PEAR(NATT)=EEN(I)
ckj              XMAR(NATT)=XMN(I)
...
cxj                   GO TO 50 !xj block, since goto 565 
                ENDIF
C                        ********Check errors
C
                nsbst=1
...
C
        ...
        DOUBLE PRECISION  dpcoal,drcoal,ecritl,xmbcut,drbmRatio
        COMMON/HSTRNG/NFP(300,15),PP(300,15),NFT(300,15),PT(300,15)
cc      SAVE /HSTRNG/
        COMMON/hjcrdn/YP(3,300),YT(3,300)
cc      SAVE /hjcrdn/
...
ctest off-8/2013:
c        write(99,*) 'a', rkp,PARJ(2),PARJ(21),HIPR1(2)
...
clin-4/2015: set upper limit for gamma_s=P(s)/P(u) to a given pslimit
...
c     symmetric splitting function as done in arXiv:1403.6321):
        PARJ(2)=min(PARJ(2),pslimit)
ctest off-8/2013:
c        write(99,*) 'b', rkp,PARJ(2),PARJ(21),HIPR1(2)
